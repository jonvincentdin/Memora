...
export function TagEditor({ resourceType, resourceId }: { resourceType: ResourceType; resourceId: string }) {
  ...
  async function add() {
    ...
    const data = await response.json().catch(() => null); if (response.ok && data?.tag) setTags((current) => current.some((tag) => tag.id === data.tag.id) ? current : [...current, data.tag]); setName("");
  }
  async function remove(tagId: string) { const response = await fetch(`/api/tags?resourceType=${resourceType}&resourceId=${resourceId}&tagId=${tagId}`, { method: "DELETE" }); if (response.ok) setTags((current) => current.filter((tag) => tag.id !== tagId)); }

...
export function AccountSettings({ initial }: { initial: { name: string; email: string } }) {
  ...
  return <section className="mt-10 border-t border-line pt-8">
    ...
    <div className="mt-8 rounded-lg border border-danger/30 p-4"><h3 className="text-sm font-semibold text-danger">Delete account</h3><p className="mt-1 text-xs text-ink-soft">This permanently removes memories, reviewers, quizzes, attempts, connections, and shared collections.</p><div className="mt-3 flex gap-2"><Input type="password" value={deletePassword} onChange={(event) => setDeletePassword(event.target.value)} placeholder="Confirm with your password" /><Button variant="danger" disabled={!deletePassword} loading={busy === "delete"} onClick={deleteAccount}>Delete</Button></div></div>
  </section>;
}

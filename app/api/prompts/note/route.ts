...
export const POST = withApiErrorHandling(async (request: Request) => {
  ...
  for (const id of noteIds) {
    ...
    if (!allowed) return NextResponse.json({ error: "One or more memories could not be found." }, { status: 404 });
  }

  const notes = await findNotesByIds(noteIds);
  if (notes.length === 0) return NextResponse.json({ error: "No matching memories found." }, { status: 404 });

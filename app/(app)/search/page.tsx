...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [notes, reviewers, quizzes] = query ? await Promise.all([
    prisma.note.findMany({ where: { ownerId: user.id, archivedAt: null, OR: [{ title: match }, { description: match }, { originalFilename: match }, { tags: { some: { tag: { name: match } } } }] }, select: { id: true, title: true, description: true, updatedAt: true, tags: { select: { tag: { select: { id: true, name: true } } } } }, orderBy: { updatedAt: "desc" }, take: 30 }),
    prisma.reviewer.findMany({ where: { ownerId: user.id, archivedAt: null, OR: [{ title: match }, { description: match }, { tags: { some: { tag: { name: match } } } }] }, select: { id: true, title: true, description: true, updatedAt: true, tags: { select: { tag: { select: { id: true, name: true } } } } }, orderBy: { updatedAt: "desc" }, take: 30 }),
    prisma.quiz.findMany({ where: { ownerId: user.id, archivedAt: null, OR: [{ title: match }, { description: match }, { tags: { some: { tag: { name: match } } } }] }, select: { id: true, title: true, description: true, updatedAt: true, tags: { select: { tag: { select: { id: true, name: true } } } } }, orderBy: { updatedAt: "desc" }, take: 30 }),
  ]) : [[], [], []];
  const sections = [
